...
int main(void){
	...
	float scale=1.0f;
	initwindow(800,800,"Robot MANUEL"); 
	...
	while(1){
			...
			***/
			...
			nDx = robot.Dr*cos(robot.Dalpha + ((nwg * Dt * R0) - (nwd * Dt * R0))/D);
			nDy = robot.Dr*sin(robot.Dalpha + ((nwg * Dt * R0) - (nwd * Dt * R0))/D);
		    ...
		    Robot_setX(&robot,Robot_getX(&robot) + robot.Dx * 5000);
		    Robot_setY(&robot,Robot_getY(&robot) + robot.Dy * 5000);
		    // Towards the Goal
